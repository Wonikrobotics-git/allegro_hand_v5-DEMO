...
int fingertip_sensor[4] = {0, 0, 0, 0};  // Fingertip 센서 값 (정수형)
...
// Rviz 손가락 끝에 'INDEX','MIDDLE','PINKY','THUMB' 문구 추가
...
void updateMarkers() {
    publishTextMarker("1", "link_3_0_tip", 0);
    publishTextMarker("2", "link_7_0_tip", 1);
    publishTextMarker("3", "link_11_0_tip", 2);
    publishTextMarker("4", "link_15_0_tip", 3);
}

/// @brief Rviz 에서 촉각센서의 값에 따른 게이지 색상 변화/ 
/// @param gauge : 게이지 
/// @param value : 촉각센서 값
...
void fingertipSensorCallback(const std_msgs::Int32MultiArray::ConstPtr& msg) {
    if (msg->data.size() >= 3) {
        ...
        sensor4_label->setText("Fingertip Sensor 4: " + QString::number(fingertip_sensor[3]) + "  Pa");
        ...
        float scaled_value4 = fingertip_sensor[3] * (255.0f / 500.0f);
        ...
        gauge4->setValue(std::min(fingertip_sensor[3],500));
      ...
      //  ros::Duration(1e-6).sleep();
        updateProgressBarColor(gauge4, scaled_value4);
    }
}
...
int main(int argc, char **argv)
{
    ...
    sensor4_label = new QLabel("Fingertip Sensor 4: 0  Pa", &window);
    ...
    gaugeLayout->addWidget(sensor4_label);
    gaugeLayout->addWidget(gauge4);

    mainLayout->addWidget(frame, 5);
