 ...
 * - Data: 2024/12/03
 * - 2024/12/03: Change several algorithm. (by Hibo)
 */
...
enum eMotionType
{
	...
	eMotionType_HOME2,				///< go to home position for grasp box object
	...
	eMotionType_GRASP_12,
	eMotionType_GRASP_23,
	eMotionType_GRASP_12_3,
...
public:
	...
	void SetPinchoffset(double x, double y, double z);
...
private:
	...
	double _qdot_pre[3][3];			///< previous(last) joint velocity (radian/sec)
	double _tau_des[3][3];			///< desired joint torque
	double _q_des[3][3];			///< desired joint position used in joint pd control motion
